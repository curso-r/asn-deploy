...
1. Testes unitários e consistência de código
...
knitr::include_graphics("img/desativar_rdata.png")
```
...
Repare que você verá o uso do `{usethis}` o tempo todo! Alguns exemplos:
...
O arquivo `README.md` é importante pois contém informações como: logo (opcional!), introdução (o que ele faz?), instruções de instalação, exemplos de uso, etc. E o Github vai apresentá-lo na página inicial do repositório.
...
O arquivo gerado é um **RMarkdown** (`.Rmd`), e portanto precisamos converter para **Markdown** (`.md`).
...
- Exemplo:`usethis::create_package("~/Documents/meupacote")` vai criar uma nova pasta chamada `meupacote` que ficará dentro da pasta `Documents`. Essa pasta será tanto um `Rproj` quanto um pacote, ambos chamados `meupacote`
...
---
# Instalando seu pacote

- Localmente: `remotes::install_local("caminho/ate/meuPacote")`

- Direto do Github: `remotes::install_github("meuUsuario/meuPacote")`

...

- Se um dia chegar ao CRAN: `install.packages("meuPacote")`
...
.footnote[Fonte: [Repositório  actions](https://github.com/r-lib/actions/tree/master/examples#quickstart-ci-workflow)]
